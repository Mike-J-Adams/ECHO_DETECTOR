...
load("C:\Users\Adamsmi\Documents\MATLAB\GITHUB\ECHO_DETECTOR\COC_EK60_VALIDATED_ALL.mat");
...
figure(2) 
subplot(3,1,1)
scatter(Freq_ranges.COCranges, Freq_ranges.FreqRMS90)
title('Outliers included')
xlabel('Horizontal Range (m)')
ylabel('RMS')
%filter out outliers
Freq_ranges_rmoutliers = Freq_ranges(Freq_ranges.COCranges <=inf,:);
subplot(3,1,2)
scatter(Freq_ranges_rmoutliers.COCranges, Freq_ranges_rmoutliers.FreqRMS90)
title('Outliers removed')
xlabel('Horizontal Range (m)')
ylabel('RMS')
subplot(3,1,3)
scatter(Freq_ranges_rmoutliers.COC_SLANT_RANGE, Freq_ranges_rmoutliers.FreqRMS90)
title('Outliers removed')
